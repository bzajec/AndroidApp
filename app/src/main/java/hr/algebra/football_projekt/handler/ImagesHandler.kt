package hr.algebra.football_projekt.handler
...
import hr.algebra.football_projekt.factory.createGetHttpUrlConection
...
fun downloadImageAndStore(context: Context, url: String, fileName: String) : String?{
    ...
    try {
        ...
        val con : HttpURLConnection = createGetHttpUrlConection(url)
        con.inputStream.use { `is` -> // reserved key -> backquote je AltGr + 7
            FileOutputStream(file).use { fos ->
                ...
                val buffer: ByteArray = getBytesFromBitmap(resizeBitmap)
                fos.write(buffer)
